class Movie < ApplicationRecord
    ...
    scope :grossed_less_than, ->(max) { released.where("total_gross < ?", max).order(total_gross: :asc) }
    scope :grossed_greater_than, ->(max) { released.where("total_gross > ?", max).order(total_gross: :asc) }
